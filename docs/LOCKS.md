...
Locks define which dimensions creative operations must preserve. They are settings schema 4/profile version 2 data, persist independently of the Garage, and are normalized on every load and write.
...
| Reroll Unlocked | current settings | Parent final is restored/verified first; only unlocked dimensions may change. An all-locked no-op is valid and still produces pending DNA. |
| Mutate | current settings | Parent final is restored/verified first; the deterministic child mutation respects current locks. Wild may change model only without model-bound locks. |
...
Fairness is evaluated only inside the eligible lock-compatible pool. A vehicle lock permits configurations of its bound model; a configuration lock permits only its exact normalized configuration. An unresolved binding blocks or reports the operation—recovery never discards it silently, though recovery itself is allowed to load an incompatible safe target and then marks the lock unresolved.

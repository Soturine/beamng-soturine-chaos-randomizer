# Owner test checklist — v0.6.6 candidate

- [ ] Verify the downloaded/local ZIP filename, bytes and SHA-256 against the report.
- [ ] Confirm only this Randomizer ZIP is enabled and header says `0.6.6-live-fix-candidate`.
...
- [ ] Update `LIVE_TEST_REPORT.md` with real results; do not approve `v0.6.6` while any required case is Failed, Pending or Blocked.

<template><nav class="scr-nav" :aria-label="t('app.name')" v-bng-scoped-nav="{ type: 'container', wrapNavigation: 'horizontal' }" v-bng-on-ui-nav:tab_l="step(-1)" v-bng-on-ui-nav:tab_r="step(1)"><button v-for="tab in tabs" :key="tab" type="button" bng-nav-item :class="{ 'is-active': active === tab }" :aria-current="active === tab ? 'page' : undefined" @click="$emit('select', tab)">{{ t(`nav.${tab}`) }}</button></nav></template>
<script setup>
